...
func articleConv(val amodels.DbArticle, auth amodels.Author) amodels.Preview {
...
func articleShortConv(val amodels.DbArticle, auth amodels.Author) (amodels.Preview, error) {
...
func fullPreviewConv(val amodels.DbArticle, Db *sqlx.DB, auth amodels.Author) (amodels.Preview, error) {
	...
	if err != nil {
		return article, sbErr.ErrDbError{
			Reason:   err.Error(),
			Function: "articleRepository/fullPreviewConv",
		}
	}
	var mytag string
	for rows.Next() {
